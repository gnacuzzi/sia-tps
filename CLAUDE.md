...
Monorepo de todos los TPs de la materia **SISTEMAS DE INTELIGENCIA ARTIFICIAL (72.27), 2026**. Cada TP vive en su propia carpeta (`tp1/`, `tp2/`, ...) y tiene su propio `CLAUDE.md` con el contexto específico.
...
Lo único que importa es que no metas arquitecturas, mecanismos o trucos de Inteligencia Artificial que no estén en el material de cátedra. Si una idea (positional encoding alternativo, tipo de atención, técnica de regularización, librería de alto nivel, etc.) no aparece en las clases o no es de sentido común para EDA/preprocesamiento tabular estándar, avisame antes de usarla — no la metas por tu cuenta aunque la conozcas de otro lado.

El material de cátedra de cada TP está en su carpeta `docs/`. Antes de implementar cualquier pieza de la arquitectura, releer las clases correspondientes y mantenerse a lo que ahí se explica.
